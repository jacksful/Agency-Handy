@if (config('layout', 'extras/user/dropdown/style') == 'light')
    ...
    <div class="d-flex align-items-center p-8 rounded-top">
        ...
        <span class="label label-light-success label-lg font-weight-bold label-inline">{{ Auth::user()->email; }}</span>
    </div>
...
@else
    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap p-8 bgi-size-cover bgi-no-repeat rounded-top" style="background-image: url('{{ asset('media/misc/bg-1.jpg') }}')">
        <div class="d-flex align-items-center mr-2">
            ...
            {{-- Text --}}
            <div class="text-white m-0 flex-grow-1 mr-3 font-size-h5">{{ Auth::user()->first_name; }} {{ Auth::user()->last_name; }}</div>
        </div>
        <span class="label label-success label-lg font-weight-bold label-inline">{{ Auth::user()->email; }}</span>
    </div>
@endif
...
<div class="navi navi-spacer-x-0 pt-5">
    {{-- Item --}}
    <a href="{{ route('my.profile') }}" class="navi-item px-8">
        <div class="navi-link">
